...
jQuery(document).ready(function() {
    ...
    jQuery(add_button).click(function(e){ //on add input button click
        ...
        if(x < max_fields){ //max input box allowed
            ...
            jQuery(wrapper).append('<div class="one" id='+x+'><input type="text" style="width:30% !important;" id="attr_values[] cssclass" name="attr_values[]"/><input type="file" name="attr_val_image[]" id="attr_val_image[]" /><a href="JavaScript:void(0);" class="remove_field" onclick="remove_field('+x+');">Remove</a></div>'); //add input box
        }
    });
    
//$(wrapper).on("click",".remove_field", function(e){ //user click on remove text

//Remove The Fields From The Text Values.
...
function remove_field(x)
{
		...
		jQuery(this).closest('.one').remove(); 
		//x--;
};
